...
func EmbedLogger() func(next http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			logger := global.NewLogger(r.Header.Get("X-Trace-Id"))
...
func (r *Router) collectHealth(isSync bool) {
	...
	for _, node := range r.balancer.OtherNodes {
		...
		go func(node *cluster.NodeInfo) {
			...
			req, err := http.NewRequest(http.MethodGet, node.BaseURL+"/healz", nil)
			if err != nil {
	...
	selfNode := r.balancer.SelfNode
	selfNode.Mu.Lock()
	selfNode.RunningHttp = selfNode.RunningHttpCounter.Load()
