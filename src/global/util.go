...
var (
	...
	RP_REQUEST_TIMEOUT      = &ResponseCode{httpCode: http.StatusRequestTimeout, msgcode: "REQUEST_TIMEOUT"}
	RP_SERVER_EXCEPTION     = &ResponseCode{httpCode: http.StatusInternalServerError, msgcode: "SERVER_EXCEPTION"}

	RP_TOO_MANY_REQUESTS       = &ResponseCode{httpCode: http.StatusTooManyRequests, msgcode: "TOO_MANY_REQUESTS"}
...
// The body is {"error": {"code": code, "message": message}}. Content-Type
// is set to application/json; charset=utf-8.
func ResponseWarn(w http.ResponseWriter, r *http.Request, responseCode *ResponseCode, message string) {
	...
	GetCtxLogger(r.Context()).Warn("ResponseWarn", "detail", message)
