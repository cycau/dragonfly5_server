...
func initSlog(nodeName string, lg global.LoggerConfig) {
	var lev slog.Level
	...
	logPath := filepath.Join(logDir, fmt.Sprintf("server.%s.log", nodeName))
...
func runServer(config global.Config) {
	...
	thisNode := &cluster.NodeInfo{
		...
		MaxHttpQueue: int32(maxHttpQueue),
	...
	server := &http.Server{
		...
		WriteTimeout:      (900 + 60) * time.Second,
...
func main() {
	...
	initSlog(config.NodeName, config.Logger)
