...
func (rh *RequestHandler) Query(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			...
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for Query")
			return
		}
...
func (rh *RequestHandler) QueryTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), false, true)
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for QueryTx")
			return
		}
		rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), true, false)
...
func (rh *RequestHandler) Execute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), false, true)
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for Execute")
			return
		}
		rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), true, false)
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
...
func (rh *RequestHandler) ExecuteTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), false, true)
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for ExecuteTx")
			return
		}
		rh.statsSetResult(dsIDX, time.Since(startTime).Milliseconds(), true, false)
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
...
func (rh *RequestHandler) StatsGet(datasourceIdx int) (runningRead int32, runningWrite int32, runningTx int32, latencyP95Ms int32, errorRate1m float64, timeoutRate1m float64) {
	stats := rh.statsInfos[datasourceIdx]

	latency := &dto.Metric{}
	stats.statLatency.WithLabelValues("p95").(prometheus.Metric).Write(latency)
	p95 := 16.0
	if quantiles := latency.GetSummary().GetQuantile(); len(quantiles) > 0 {
		if v := quantiles[0].GetValue(); !math.IsNaN(v) {
			p95 = v
		}
	}
	...
	return runningRead, runningWrite, runningTx, int32(p95), errorRate1m, timeoutRate1m
}
...
func (rh *RequestHandler) BeginTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == context.DeadlineExceeded {
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for BeginTx")
			return
		}
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
		return
...
func (rh *RequestHandler) CommitTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for CommitTx")
			return
		}
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
		return
...
func (rh *RequestHandler) RollbackTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for RollbackTx")
			return
		}
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
		return
...
func (rh *RequestHandler) CloseTx(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			ResponseError(w, r, RP_REQUEST_TIMEOUT, "Request timeout for CloseTx")
			return
		}
		ResponseError(w, r, RP_DATASOURCE_EXCEPTION, err.Error())
		return
