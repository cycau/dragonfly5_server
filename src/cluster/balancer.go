...
func (b *Balancer) SelectNode(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		// RunningHttp count
		current := b.SelfNode.RunningHttpCounter.Add(1)
		if current > b.SelfNode.MaxHttpQueue {
			b.SelfNode.RunningHttpCounter.Add(-1) // realtime decrement!
			global.ResponseError(w, r, RP_TOO_MANY_REQUESTS, fmt.Sprintf("No resource to process on this node. Node[%s], RunningHttp %d/%d", b.SelfNode.NodeID, current, b.SelfNode.MaxHttpQueue))
			return
		}
		defer b.SelfNode.RunningHttpCounter.Add(-1)
		...
		if txID != "" {
			...
			if dsIdx >= len(b.SelfNode.Datasources) || dsIdx < 0 {
				http.Error(w, fmt.Sprintf("Invalid datasource index from txID: %d", dsIdx), http.StatusBadRequest)
				return
			}
		...
		if redirectCount < 1 {
			...
			global.ResponseError(w, r, RP_DATASOURCE_UNAVAILABLE, fmt.Sprintf("No resource to process on this node. Node[%s], RedirectCount exceeded", b.SelfNode.NodeID))
			return
		}
		...
		if recommendNode == nil {
			...
			global.ResponseError(w, r, RP_DATASOURCE_UNAVAILABLE, fmt.Sprintf("No resource to process on this node. Node[%s], and other nodes seem to be not available too.", b.SelfNode.NodeID))
			return
		}

		// 自分のScoreの方が高い場合は自分で処理
		if selfBestScore != nil && selfBestScore.score >= recommendNodeScore.score {
		...
		global.ResponseWarn(w, r, RP_RECOMMEND_OTHER_NODE, fmt.Sprintf("Redirecting to other node %s from %s", recommendNode.NodeID, b.SelfNode.NodeID))
	}
...
var randPool = sync.Pool{
	New: func() any { return rand.New(rand.NewPCG(rand.Uint64(), rand.Uint64())) },
}
...
func selectBestRandomScore(scores []*ScoreWithWeight) (bestScore *ScoreWithWeight, bestRandomScore *ScoreWithWeight) {
	...
	best := topK[0]
	totalWeight := best.score * best.weight
	for _, sc := range topK[1:] {
	...
	// 浮動小数点誤差でループを抜けた場合、最後の有効要素をフォールバック
	return best, topK[len(topK)-1]
}
