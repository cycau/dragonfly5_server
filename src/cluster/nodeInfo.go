...
type NodeInfo struct {
	...
	MaxHttpQueue       int32            `json:"maxHttpQueue"`
	RunningHttp        int32            `json:"runningHttp"`
	UpTime             time.Time        `json:"upTime"`
	CheckTime          time.Time        `json:"checkTime"`
	Datasources        []DatasourceInfo `json:"datasources"`
	RunningHttpCounter atomic.Int32     `json:"-"`
	Mu                 sync.Mutex       `json:"-"`
}
...
type DatasourceInfo struct {
	...
	RunningRead   int32   `json:"runningRead"`
	RunningWrite  int32   `json:"runningWrite"`
	RunningTx     int32   `json:"runningTx"`
...
func (node *NodeInfo) Clone() NodeInfo {
	...
	return NodeInfo{
		...
		RunningHttp:  node.RunningHttpCounter.Load(),
		UpTime:       node.UpTime,
...
func (node *NodeInfo) GetScore(ctx context.Context, tarDbName string, endpoint global.ENDPOINT_TYPE) []*ScoreWithWeight {
	...
	for dsIdx := range node.Datasources {
		...
		if endpoint == global.EP_Execute && dsInfo.MaxWriteConns < 1 {
			continue
		}
		if endpoint == global.EP_BeginTx && dsInfo.MaxWriteConns < 1 {
			continue
		}
		...
		case global.EP_BeginTx:
			weight = float64(dsInfo.MaxWriteConns * 7 / 10)
		default:
...
func calculateMetrics(node *NodeInfo, dsInfo *DatasourceInfo) normalizedMetrics {
	...
	txCap := float64(dsInfo.MaxWriteConns*7/10) * 2
